...
const std::vector<double> PLASMA = {0.050383, 0.029803, 0.527975,  //
...
const std::vector<double> MAGMA = {0.001462, 0.000466, 0.013866,  //
...
const std::vector<double> INFERNO = {0.001462, 0.000466, 0.013866,  //
...
const std::vector<double> VIRIDIS = {0.267004, 0.004874, 0.329415,  //
...
static void interpolated_color(double val, const double *cmap, double *color) {
  double normalized_val = (NCOLORS - 2) * val;
  size_t i0 = normalized_val;
  size_t i1 = (double)1 + normalized_val;
...
static void nearest_color(double val, const double *cmap, double *color) {
  // round to nearest integer
  size_t i = (NCOLORS - 1.0) * val + 0.5;
  color[0] = cmap[i * 3];
  color[1] = cmap[i * 3 + 1];
  color[2] = cmap[i * 3 + 2];
}

auto get_interpolated_colors(std::vector<double> &values,
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    interpolated_color((values[i] - minv) * inv_delta, cmap.data(),
                       &colors[i * 3]);
  }
  return colors;
}
...
auto get_nearest_colors(std::vector<double> &values,
  ...
  std::vector<double> colors(values.size() * 3);
  for (size_t i = 0; i < values.size(); ++i) {
    nearest_color((values[i] - minv) * inv_delta, cmap.data(), &colors[i * 3]);
  }
  return colors;
}
